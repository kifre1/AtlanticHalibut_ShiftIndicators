...
# Ensure that NAFO_Zones is a factor ordered from south to north
south_to_north <- c("5YZ6A", "4X", "4VW", "4RST", "3NOPs", "3KL", "2JHG")
halcrib_with_zones_clean1$NAFO_Zones <- factor(halcrib_with_zones_clean1$NAFO_Zones, levels = south_to_north)

# Revised function for boxplot with flipped orientation of axes
plot_single_index_with_gradient <- function(data, x_col, title, y_label, gradient_direction) {
  ggplot(data, aes(y = NAFO_Zones, x = !!sym(x_col))) +
    # Add a faded background gradient, aligned with the x-axis
    geom_tile(
      data = expand.grid(
        NAFO_Zones = unique(data$NAFO_Zones),    # Category groups remain on y-axis
        x = seq(0, 1, length.out = 500)         # Continuous variable spans the x-axis
      ),
      aes(y = NAFO_Zones, x = x, fill = x, alpha = 0.3),  # Add gradient with alpha
      inherit.aes = FALSE
    ) +
    # Use a custom background gradient
    scale_fill_gradientn(
      colors = viridis::viridis(100, direction = gradient_direction),  # Flip colors based on direction
      limits = c(0, 1),
      guide = "none"  # Remove legend for the gradient
    ) +
    # Add the boxplot visualization
    geom_boxplot(fill = "grey50", color = "black", outlier.shape = NA, alpha = 0.8) +
    # Add labels and themes
    labs(
      title = title,
      x = "Value",  # Generic label for the x-axis
      y = y_label   # Label for the y-axis
    ) +
    theme_minimal() +
    theme(
      axis.text.y = element_text(angle = 0, hjust = 1),  # Align NAFO zone labels vertically
      plot.title = element_text(size = 14, face = "bold")
    )
}

# Single Plot: AC.Thermal.habitat.availability (gradient: purple at 0, yellow at 1)
plot_ac_thermal_habitat <- plot_single_index_with_gradient(
  halcrib_with_zones_clean1,
  x_col = "AC.Thermal.habitat.availability",
  title = "Thermal Adaptability (THV)",
  y_label = "NAFO Zones (South to North)",
  gradient_direction = -1  # Purple at 0, yellow at 1
)

# Single Plot: S.Thermal.safety.margin (gradient: yellow at 0, purple at 1)
plot_s_thermal_safety <- plot_single_index_with_gradient(
  halcrib_with_zones_clean1,
  x_col = "S.Thermal.safety.margin",
  title = "Thermal Sensitivity (TSM)",
  y_label = "NAFO Zones (South to North)",
  gradient_direction = 1  # Yellow at 0, purple at 1
)

# Display the plots
print(plot_ac_thermal_habitat)
print(plot_s_thermal_safety)

# Boxplot for ToE.year (one per SSP scenario)
# Create the background gradient
background_gradient <- ggplot() +
  # Add background gradient using geom_tile()
  geom_tile(
    data = expand.grid(
      x = seq(2015, 2100, length.out = 100),  # ToE.year intervals (x-axis)
      y = seq(0, 1, length.out = 100)        # Normalized y-axis
    ),
    aes(x = x, y = y, fill = x),
    inherit.aes = FALSE,
    alpha=0.3
  ) +
  # Define the gradient from yellow (2015) to purple (2100)
  scale_fill_gradientn(
    colors = viridis(100, direction = -1),  # Gradient: yellow to purple
    limits = c(2015, 2100),                # Range matches ToE.year
    guide = "none"                         # Remove gradient legend
  ) +
  coord_cartesian(expand = TRUE) +           # Ensure tight bounding box
  theme_void() +                              # No axes or gridlines
  theme(plot.margin = unit(c(0.06, 2.7, 0.3, 1.3), "cm")) #top, right, bottom, left

# Ensure NAFO_Zones is a factor ordered from south to north
south_to_north <- c("5YZ6A", "4X", "4VW", "4RST", "3NOPs", "3KL", "2JHG")
halcrib_with_zones_clean1$NAFO_Zones <- factor(
  halcrib_with_zones_clean1$NAFO_Zones,
  levels = south_to_north
)

# Create the boxplot layer
boxplot_toe_year <- ggplot(halcrib_with_zones_clean1, aes(y = NAFO_Zones, x = ToE.year, fill = ssp)) +
  geom_boxplot(
    color = "black",                     # Black outlines for boxes
    outlier.shape = NA,                  # Remove outlier points
    alpha = 1,                           # Solid boxplot fill
    position = position_dodge(width = 0.7)  # Side-by-side SSPs
  ) +
  # Assign greyscale colours for SSPs
  scale_fill_manual(
    values = c("SSP1-2.6" = "grey80", "SSP5-8.5" = "grey40"),  # Grey tones
    name = "SSP Scenario"                                     # Legend title
  ) +
  # Add axis labels, title, and limits
  labs(
    title = "Time of Emergence (ToE Year)",
    x = "Year of Climate Emergence",
  ...
  theme(
    ...
    axis.text.y = element_text(angle = 0, hjust = 1),                  # Properly aligned y-axis
...
# Combine the background gradient and the boxplot
final_plot <- ggdraw() +
  draw_grob(ggplotGrob(background_gradient)) +   # Add the background gradient
  draw_grob(ggplotGrob(boxplot_toe_year))        # Overlay the boxplot on top
...
combined_plot <- plot_grid(
  plot_s_thermal_safety,
  plot_ac_thermal_habitat,
  final_plot,
  nrow = 1,  # 1 row
  ncol = 3,  # 3 columns
